...
class ResendVerifyPassScreen extends StatefulWidget {
  @override
  _ResendVerifyPassScreenState createState() => _ResendVerifyPassScreenState();
}

class _ResendVerifyPassScreenState extends State<ResendVerifyPassScreen> {
  final GlobalKey<FormState> _resendVerifyPassFormKey = GlobalKey<FormState>();
  ...
  void doValidate() {
    if (_resendVerifyPassFormKey.currentState.validate()) {
      _resendVerifyPassFormKey.currentState.save();
      resendVerifyEmail();
    } else {
      setState(() {
        _autoValidate = true;
  ...
  void resendVerifyEmail() {
    pr.show();
    Map body = {"email_address": _email};
    // print(body);
    WebService api = WebService();
    final response = api.callPostAPI(RESEND_VERIFICATION_EMAIL, body, '');
    response.then((value) {
      //print(value.body);
      if (pr.isShowing()) {
        pr.hide();
  ...
  Widget resendVrificationFormUI() {
    return Column(
      children: <Widget>[
        SizedBox(
          height: 20,
    ...
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: AppBar(
          ...
          title: Container(
            child: Text(
              AppTranslations.of(context).text('resend_verification_email'),
              style: TextStyle(
                color: Colors.white,
      ...
      body: Container(
        child: Center(
          child: new Form(
            key: _resendVerifyPassFormKey,
            autovalidate: _autoValidate,
            child: resendVrificationFormUI(),
          ),
        ),
      ),
    );
